...
model_measles |>
  add_globalevent({
    InterventionMeaslesPEP(
      ...
      mmr_window = 3,
      quarantine_states         = c(
        7L,  # Quarantined Exposed
        # 8L # Quarantined Susceptible (see note)
        9L   # Quarantined Prodromal
      ),
      quarantine_states_for_pep = c(
        0L,  # Susceptible
        # 0L # Susceptible (see note)
        12L  # Recovered (for exposed agents for which PEP is effective)
      )
    )
  })
...
Although PEP is not beign applied to susceptible agents at first, since this intervention runs on a daily basis, agents transitioning to Quarantined Exposed may receive PEP as well. If the user wanted to evaluate the impact of PEP in a model using contact tracing, they could use this same intervention in the `ModelMeaslesMixingRiskQuarantine` model, which has a more detailed quarantine process that moves only exposed agents to quarantine states.

::: {.callout-note title="When to use PEP in the model?"}
During the development of the intervention, we observed that the MMR was beign applied signigicantly more frequently than IG, which is not consistent with real-world practices. This is mostly due to the fact that the quarantine process implemented in the model moves both exposed and susceptible individuals to quarantine states. Since the number of exposed agents is significantly lower than the number of susceptible agents, the PEP intervention is mostly being applied to susceptible agents, for which the MMR is distributed.
:::
