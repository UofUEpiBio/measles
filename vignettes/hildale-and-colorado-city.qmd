---
title: "Mixing model in Hildale and Colorado City"
author: "George G. Vega Yon, Ph.D."
date: today
vignette: >
  %\VignetteIndexEntry{Mixing model in Hildale and Colorado City}
  %\VignetteEngine{quarto::html}
  %\VignetteEncoding{UTF-8}
---

::: {.callout-warning title="Warning"}
The data currently used in this vignette is partly fictional and for demonstration purposes only. Please take that into consideration when interpreting the results. The current vaccination rates in the Hildale and Colorado City region will most certainly differ.
:::

During the 2025 US Measles outbreak, the number of cases in the state of Utah started to climb quickly in the Hildale and Colorado City region (on the Utah-Arizona border). This vignette shows how we can use the mixing model of the `measles` R package to do scenario modeling of the situation using both age and school data.
...
```{r}
#| label: data
data(hildale_and_colorado_city, package = "measles")
data(hildale_and_colorado_city_matrix, package = "measles")
```
...
hildale_and_colorado_city_matrix[, 1:5] |>
  round(2)
```

One important thing to note is that the data in `hildale_and_colorado_city` must coincide with that in the contact matrix, particularly, there should be one entry in the `data.frame` for each row/column of the contact matrix.

## Creating the model

::: {.callout-warning title="Warning"}
The current parameters, particularly those related to transmission, have not been calibrated in this version of the vignette. Please take that into consideration when interpreting the results, which has a basic reproductive number of around 15.
:::
...
```{r}
#| label: create-model
N <- sum(hildale_and_colorado_city$agepops)
...
measles_model <- ModelMeaslesMixing(
  ...
  vax_efficacy                 = 0.97,
  contact_matrix               = hildale_and_colorado_city_matrix,
...
add_entities_from_dataframe(
  model = measles_model,
  entities = hildale_and_colorado_city,
  col_name = "age_labels",
  col_number = "agepops",
  as_proportion = FALSE
)
...
set_distribution_tool(
  ...
  distfun = distribute_tool_to_entities(
    prevalence = hildale_and_colorado_city$vacc_rate,
    as_proportion = TRUE
  )
)
```
...
The function call will get us the results as a list of data.frames (data.table objects in this case). Since we are only saving the `"virus_hist"`, we can access that item from the data and, for convenience, process it using the `data.table` package:
