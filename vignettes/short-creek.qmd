...
For calibration, we can make use of the function `calibrate_mixing_model()`, which will give us the needed scaleing factor to apply to the contact matrix to get the desired R0. In this case, we will use an R0 of 12, which is consistent with the literature on measles.
...
measles_model <- ModelMeaslesMixing(
  ...
  vax_efficacy                 = 0.97,
  contact_matrix               = short_creek_matrix * scaling_factor,
