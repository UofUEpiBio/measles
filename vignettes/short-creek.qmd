...
measles_model <- ModelMeaslesMixing(
  ...
  vax_efficacy                 = 0.97,
  contact_matrix               = short_creek_matrix * .8,
...
measles_model |>
  run_multiple(
    ...
    nsims = 400,
    seed  = 8812,
    saver = make_saver("outbreak_size", "hospitalizations", "transmission"),
    nthreads = 2L
  )
```
...
transmission <- ans$transmission |> as.data.table()
```
...
r_eff <- transmission[
  source_exposure_date == 0 &
    source != -1
][, .N, by = .(sim_num)]

hist(r_eff$N, main = "Effective reproductive number", xlab = "R_eff", breaks = 20)
